...
let ``Auto generateSerializer works for lists`` () =
    ...
    serializer [ false; true; true; false; false ]
    |> equals (ListValue [ f; t; t; f; f ])
...
let ``Auto generateSerializer works for seq`` () =
    ...
    |> equals (ListValue [ IntValue 1; IntValue 2; IntValue 3 ])
...
let ``Auto generateSerializer works for arrays`` () =
    ...
    serializer [| 6; 7 |]
    |> equals (ListValue [ IntValue 6; IntValue 7 ])
...
let ``Auto generateSerializer works for DateTime`` () =
    ...
    let dt =
        DateTime.Parse ("2022-09-30T22:04:30.8560000Z", null, Globalization.DateTimeStyles.RoundtripKind)

    serializer dt
    |> equals (StringValue "2022-09-30T22:04:30.8560000Z")

[<Fact>]
let ``Auto generateSerializer works for Guid`` () =
    let serializer = Auto.generateSerializer<Guid> ()

    serializer (Guid.Parse ("2fa55ac7-435b-46f0-9f5b-a6887582da30"))
    |> equals (StringValue "2fa55ac7-435b-46f0-9f5b-a6887582da30")
...
let ``Auto generateSerializer works for sets`` () =
    ...
    serializer (Set.ofSeq [ 1; 2; 4 ])
    |> equals (ListValue [ IntValue 1; IntValue 2; IntValue 4 ])

[<Fact>]
let ``Auto generateSerializer works for Map<string, _>`` () =
    let serializer = Auto.generateSerializer<Map<string, int>> ()

    let m = Map.ofSeq [ "abc", 123; "def", 456 ]

    serializer m
    |> equals (ObjectValue (Map.ofSeq [ "abc", IntValue 123; "def", IntValue 456 ]))
